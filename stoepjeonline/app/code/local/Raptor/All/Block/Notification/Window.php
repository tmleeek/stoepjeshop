...
class Raptor_All_Block_Notification_Window extends Mage_Adminhtml_Block_Notification_Window
{
	const ICON_PATH     = 'rall/feed/icon/path';
	
    public function getSeverityIconsUrl()
    {
        if (is_null($this->_severityIconsUrl)) {
            if (strstr($this->getNoticeMessageText(), "Raptor")) {
            	$this->_severityIconsUrl = Mage::getStoreConfig(self::ICON_PATH);
            } else {
