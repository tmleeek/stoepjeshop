...
//        'product_sku',
    ...
    /**
     * Check version
     *  - Synonym of Mage::helper('advancedreports')->checkVersion($version)
     * 
     * @param string $version Version to check
     * @return boolean
     */
    protected function _checkVer($version)
    {
        return Mage::helper('advancedreports')->checkVersion($version);
    }
    ...
    {
        if ($this->_checkVer('1.10.0.2')){
            $this->getCollection()->getSelect()
                    ->where("main_table.store_id in ('".implode("','", $storeIds)."')");            
        } elseif ($this->_checkVer('1.7.1.0') && !$this->_checkVer('1.8.0.0')){
            $this->getCollection()->getSelect()
     ...
     * @param arrau $data Associative array
     * @return string 
     */
