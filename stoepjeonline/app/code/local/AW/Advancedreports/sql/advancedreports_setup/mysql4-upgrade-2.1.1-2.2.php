...
CREATE TABLE IF NOT EXISTS `{$this->getTable('aw_arep_sku_relevance')}`(
   ...
   PRIMARY KEY (`entity_id`),
   KEY `FK_AREP_VARCHAR_PRODUCT_SKU` (`sku`),
   CONSTRAINT `FK_AREP_VARCHAR_PRODUCT_SKU` FOREIGN KEY (`sku`) REFERENCES `{$this->getTable('catalog_product_entity')}` (`sku`) ON DELETE CASCADE ON UPDATE CASCADE
 )  Engine=InnoDB charset=utf8;
