...
class EM_Productsfilterwidget_Model_Productrule extends Mage_CatalogRule_Model_Rule
{
    ...
    {
      return Mage::getModel('productsfilterwidget/rule_condition_product');
    }
    ...
    {
		$arrAttribute=Mage::registry('arrAttribute');
		$visibility = array(
        ...
        if (is_null($this->_productIds)) {
			...
			$productCollection->addAttributeToFilter('status',1)->addAttributeToSelect('*')->addStoreFilter(Mage::app()->getStore()->getId());
				if($arrAttribute){	
				...
				$productCollection->setPageSize(500);
				$tong	=	count($productCollection->getData());
				...
				{				
					...
					$productCollection->getSelect()->limit($tong,$offset);
    ...
    {		
		...
		$object->setData('product', $product);
		
		$catalogRule=Mage::registry('catalogRule');	
