...
class PayNL_Giropay_Model_Api_Giropay extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 694; // 694 = giropay
    const     API_URL = "https://api.pay.nl:443/xmlrpc.php"; // xmlrpc service
    const     API_CHECK_BANKCODE = "https://api.pay.nl/controllers/payments/giropay.php?act=bankcheck&bankCode=";
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/giropay_giropay/'.$key);
    ...
    private function validateBankId($bankId)
    {
        // Do API GET (with cURL, to not break the standard)
        $curl=curl_init(self::API_CHECK_BANKCODE.$bankId);
    ...
    public function createPayment ($bank_id="", $amount, $return_url, $report_url, $emailAddress, $orderData)
    {
        ...
        // Preprocess for stupid users
        $bank_id = trim($bank_id);
        $bank_id = str_replace(' ', '', $bank_id);
        $bank_id = str_replace('.', '', $bank_id);

        if (!is_null($bank_id) && strlen($bank_id) > 0)
        {
            if (!$this->validateBankId($bank_id))
            {
                return false;
            }
        }
