 ...
 */
class PayNL_Mrcash_Model_Mrcash extends Mage_Payment_Model_Method_Abstract
{
    const PAYMENT_TYPE_SALE = 'SALE';
    protected $_code  = 'mrcash_mrcash';
    protected $_formBlockType = 'mrcash/mrcash_form';
    protected $_infoBlockType = 'mrcash/mrcash_info';
    ...
    {
          return Mage::getUrl('mrcash/mrcash/redirect', array('_secure' => true));
    }

    /**
     * Get Pay.nl mrcash api Model
     *
     * @return PayNL_Mrcash_Model_Api_Mrcash
     */
    public function getApi()
    {
        return Mage::getSingleton('mrcash/api_mrcash');
    }

    /**
     * Make sure only the correct currency's are allowed for mrcash...
     * That being EUR only.
     *
     * @return bool
     */
    public function validate()
    {
        ...
        if (!in_array($currency_code,$this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('mrcash')->__('Selected currency code ('.$currency_code.') is not compatible with MisterCash'));
        }
        return $this;
    }
            ...
            else
            {
                ...
                if (in_array($action,$arrActionDel))
                {
                    ...
                    $order->addStatusToHistory(
                                    ...
                                    Mage::helper('mrcash')->__('Refund', true),
                                    false);
                    ...
                    $mail->setSubject(Mage::helper('mrcash')->__('Order #%s refunded', $order->getRealOrderId()));
                    $mail->setFrom($fromaddr,$fromname);

                    $body = Mage::helper('mrcash')->__("The order: #%s has been refunded.\nPay.nl information:\nOrder code: %s\nNo further notifications for this will be available.",$order->getRealOrderId(),$transactionId);
                    $mail->setBodyText($body);
                    $mail->send();
                    return true;
                }
                $order->setMrcashPaidStatus(1);
                $orderAmount = $order->getBaseGrandTotal()*100;
                $mrcashAmount = $response->getAmount();
                if ($mrcashAmount!=$orderAmount && false)
                {
                    // TODO: create more stuff to handle this.
                    $order->addStatusToHistory(
                        $order->getStatus(),//continue setting current order status
                        Mage::helper('mrcash')->__('Order total amount does not match mrcash gross total amount')
                    );
                    $order->save();
                }
                else
                {
                    ...
                    if (!$order->canInvoice()) {
                       ...
                       $order->setState(
                           ...
                           Mage::helper('mrcash')->__('Invoice #%s created', $invoice->getIncrementId()),
                           $notified = true
                       );
                    }
                    $order->save();
    ...
    public function getReturnUrl() {
        return Mage::getUrl('mrcash/mrcash/return');
    }
    
    public function getReportUrl() {
        return Mage::getUrl('mrcash/mrcash/report');
    }
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $stateObject->setStatus('pending_mrcash');
