...
class PayNL_Mrcash_Model_Api_Mrcash extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 436; // 436 = mrcash
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/mrcash_mrcash/'.$key);
