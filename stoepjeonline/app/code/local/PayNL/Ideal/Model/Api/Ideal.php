...
class PayNL_Ideal_Model_Api_Ideal extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 10; // 10 = iDeal
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/ideal_ideal/'.$key);
    ...
    {
        $banks_tmp = $this->_doApiCall('transaction.idealGetBanks',array());
        if (is_array($banks_tmp))
        {
            $banks_array = array();

            foreach ($banks_tmp['banks'] as $bank)
            {
                $banks_array[$bank["issuerId"]] = $bank['name'];
            }
            return $banks_array;
        }
        else
    ...
    public function createPayment ($bank_id = "", $amount, $return_url, $report_url, $emailAddress, $orderData)
    {
        ...
        if (strlen($this->getBankId()) > 0)
        {
            $arguments['issuerId'] = $this->getBankId();
        }
	...
	if ($this->testmode == true)
	{
		...
		$stores = Mage::getModel("storepicker/store")->load($storepickerId);

		$storepickerCustomerno = $stores->getCustomerno();
		//$storepickerImportid = $stores->getImportid();

		$arguments['extra1'] = $storepickerCustomerno;//.' '.$storepickerImportid;
		$arguments['extra2'] = $orderData['order']['increment_id'];
