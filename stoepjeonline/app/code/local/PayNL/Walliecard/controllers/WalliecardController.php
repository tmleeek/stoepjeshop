...
class PayNL_Walliecard_WalliecardController extends Mage_Core_Controller_Front_Action
{
    ...
    public function getWalliecard()
    {
        return Mage::getSingleton('walliecard/walliecard');
    }
    /*
    ...
    public function redirectAction()
    {
        ...
        if($order->getId())
        {
            if(strlen($order->getWalliecardTransactionId()) == 0)
            {
              ...
              if ($response)
              {
                  $session->setWalliecardWalliecardQuoteId($session->getQuoteId());
                  $order->setWalliecardTransactionId($response->getTransactionId());
                  $order->setWalliecardPaidStatus(0);
                  $order->save();

                  $this->getResponse()->setBody(
                      $this->getLayout()->createBlock('walliecard/walliecard_redirect')
	...
	$orderApi = Mage::getSingleton('walliecard/api_walliecard')->checkPayment($transactionId);
        $order = Mage::getModel('sales/order');
        ...
        if ($orderApi->getPaidStatus() != true)
        {
            ...
            $session->setQuoteId($session->getWalliecardWalliecardQuoteId(true));
	...
	$transaction_id = $this->getRequest()->get('order_id');
        ...
        $this->getWalliecard()->checkPayment($transaction_id,$action);
	echo "true|done";
    }
