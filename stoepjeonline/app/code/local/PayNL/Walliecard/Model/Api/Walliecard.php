...
class PayNL_Walliecard_Model_Api_Walliecard extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 1.18 euro
    const     MAX_TRANS_AMOUNT = 25000; // 250.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 135; // 135 = wallie
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/walliecard_walliecard/'.$key);
