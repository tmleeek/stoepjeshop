 ...
 */
class PayNL_Maestro_Model_Maestro extends Mage_Payment_Model_Method_Abstract
{
    const PAYMENT_TYPE_SALE = 'SALE';
    protected $_code  = 'maestro_maestro';
    protected $_formBlockType = 'maestro/maestro_form';
    protected $_infoBlockType = 'maestro/maestro_info';
    ...
    {
          return Mage::getUrl('maestro/maestro/redirect', array('_secure' => true));
    }

    /**
     * Get Pay.nl maestro api Model
     *
     * @return PayNL_Maestro_Model_Api_Maestro
     */
    public function getApi()
    {
        return Mage::getSingleton('maestro/api_maestro');
    }

    /**
     * Make sure only the correct currency's are allowed for maestro...
     * That being EUR only.
     *
     * @return bool
     */
    public function validate()
    {
        ...
        if (!in_array($currency_code,$this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('maestro')->__('Selected currency code ('.$currency_code.') is not compatible with MisterCash'));
        }
        return $this;
    }
            ...
            else
            {
                ...
                if (in_array($action,$arrActionDel))
                {
                    ...
                    $order->addStatusToHistory(
                                    ...
                                    Mage::helper('maestro')->__('Refund', true),
                                    false);
                    ...
                    $mail->setSubject(Mage::helper('maestro')->__('Order #%s refunded', $order->getRealOrderId()));
                    $mail->setFrom($fromaddr,$fromname);

                    $body = Mage::helper('maestro')->__("The order: #%s has been refunded.\nPay.nl information:\nOrder code: %s\nNo further notifications for this will be available.",$order->getRealOrderId(),$transactionId);
                    $mail->setBodyText($body);
                    $mail->send();
                    return true;
                }
                $order->setMaestroPaidStatus(1);
                $orderAmount = $order->getBaseGrandTotal()*100;
                $maestroAmount = $response->getAmount();
                if ($maestroAmount!=$orderAmount && false)
                {
                    // TODO: create more stuff to handle this.
                    $order->addStatusToHistory(
                        $order->getStatus(),//continue setting current order status
                        Mage::helper('maestro')->__('Order total amount does not match maestro gross total amount')
                    );
                    $order->save();
                }
                else
                {
                    ...
                    if (!$order->canInvoice()) {
                       ...
                       $order->setState(
                           ...
                           Mage::helper('maestro')->__('Invoice #%s created', $invoice->getIncrementId()),
                           $notified = true
                       );
                    }
                    $order->save();
    ...
    public function getReturnUrl() {
        return Mage::getUrl('maestro/maestro/return');
    }
    
    public function getReportUrl() {
        return Mage::getUrl('maestro/maestro/report');
    }
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $stateObject->setStatus('pending_maestro');
