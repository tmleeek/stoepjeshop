...
class PayNL_Maestro_Model_Api_Maestro extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 712; // 712 = maestro
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/maestro_maestro/'.$key);
