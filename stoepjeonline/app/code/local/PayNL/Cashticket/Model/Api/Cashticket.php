...
class PayNL_Cashticket_Model_Api_Cashticket extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 100000; // 1000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 550; // 550 = cashticket
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/cashticket_cashticket/'.$key);
