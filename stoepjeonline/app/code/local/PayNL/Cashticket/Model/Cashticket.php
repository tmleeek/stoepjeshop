<?php
/**
 * Pay.nl Cashticket checkout model
 *
 * @category    PayNL
 ...
 */
class PayNL_Cashticket_Model_Cashticket extends Mage_Payment_Model_Method_Abstract
{
    const PAYMENT_TYPE_SALE = 'SALE';
    protected $_code  = 'cashticket_cashticket';
    protected $_formBlockType = 'cashticket/cashticket_form';
    protected $_infoBlockType = 'cashticket/cashticket_info';
    ...
    {
          return Mage::getUrl('cashticket/cashticket/redirect', array('_secure' => true));
    }

    /**
     * Get Pay.nl cashticket api Model
     *
     * @return PayNL_Cashticket_Model_Api_Cashticket
     */
    public function getApi()
    {
        return Mage::getSingleton('cashticket/api_cashticket');
    }

    /**
     * Make sure only the correct currency's are allowed for cashticket...
     * That being EUR only.
     *
     * @return bool
     */
    public function validate()
    {
        ...
        if (!in_array($currency_code,$this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('cashticket')->__('Selected currency code ('.$currency_code.') is not compatible with CashTicket'));
        }
        return $this;
    }
            ...
            else
            {
                ...
                if (in_array($action,$arrActionDel))
                {
                    ...
                    $order->addStatusToHistory(
                                    ...
                                    Mage::helper('cashticket')->__('Refund', true),
                                    false);
                    ...
                    $mail->setSubject(Mage::helper('cashticket')->__('Order #%s refunded', $order->getRealOrderId()));
                    $mail->setFrom($fromaddr,$fromname);

                    $body = Mage::helper('cashticket')->__("The order: #%s has been refunded.\nPay.nl information:\nOrder code: %s\nNo further notifications for this will be available.",$order->getRealOrderId(),$transactionId);
                    $mail->setBodyText($body);
                    $mail->send();
                    return true;
                }
                $order->setCashticketPaidStatus(1);
                $orderAmount = $order->getBaseGrandTotal()*100;
                $cashticketAmount = $response->getAmount();
                if ($cashticketAmount!=$orderAmount && false)
                {
                    // TODO: create more stuff to handle this.
                    $order->addStatusToHistory(
                        $order->getStatus(),//continue setting current order status
                        Mage::helper('cashticket')->__('Order total amount does not match cashticket gross total amount')
                    );
                    $order->save();
                }
                else
                {
                    ...
                    if (!$order->canInvoice()) {
                       ...
                       $order->setState(
                           ...
                           Mage::helper('cashticket')->__('Invoice #%s created', $invoice->getIncrementId()),
                           $notified = true
                       );
                    }
                    $order->save();
    ...
    public function getReturnUrl() {
        return Mage::getUrl('cashticket/cashticket/return');
    }
    
    public function getReportUrl() {
        return Mage::getUrl('cashticket/cashticket/report');
    }
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $stateObject->setStatus('pending_cashticket');
