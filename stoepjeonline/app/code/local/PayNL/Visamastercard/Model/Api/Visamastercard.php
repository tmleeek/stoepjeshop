...
class PayNL_Visamastercard_Model_Api_Visamastercard extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 706; // 706 = visamastercard for ecommerce
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/visamastercard_visamastercard/'.$key);
