<?php
/**
 * Pay.nl Visamastercard checkout model
 *
 * @category    PayNL
 ...
 */
class PayNL_Visamastercard_Model_Visamastercard extends Mage_Payment_Model_Method_Abstract
{
    const PAYMENT_TYPE_SALE = 'SALE';
    protected $_code  = 'visamastercard_visamastercard';
    protected $_formBlockType = 'visamastercard/visamastercard_form';
    protected $_infoBlockType = 'visamastercard/visamastercard_info';
    ...
    {
          return Mage::getUrl('visamastercard/visamastercard/redirect', array('_secure' => true));
    }

    /**
     * Get Pay.nl api Model
     *
     * @return PayNL_Visamastercard_Model_Api_Visamastercard
     */
    public function getApi()
    {
        return Mage::getSingleton('visamastercard/api_visamastercard');
    }

    /**
     * Make sure only the correct currency's are allowed for visamastercard...
     * That being EUR only.
     *
     * @return bool
     */
    public function validate()
    {
        ...
        if (!in_array($currency_code,$this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('visamastercard')->__('Selected currency code ('.$currency_code.') is not compatible'));
        }
        return $this;
    }
            ...
            else
            {
                ...
                if (in_array($action,$arrActionDel))
                {
                    ...
                    $order->addStatusToHistory(
                                    ...
                                    Mage::helper('visamastercard')->__('Refund', true),
                                    false);
                    ...
                    $mail->setSubject(Mage::helper('visamastercard')->__('Order #%s refunded', $order->getRealOrderId()));
                    $mail->setFrom($fromaddr,$fromname);

                    $body = Mage::helper('visamastercard')->__("The order: #%s has been refunded.\nPay.nl information:\nOrder code: %s\nNo further notifications for this will be available.",$order->getRealOrderId(),$transactionId);
                    $mail->setBodyText($body);
                    $mail->send();
                    return true;
                }
                $order->setVisamastercardPaidStatus(1);
                $orderAmount = $order->getBaseGrandTotal()*100;
                $visamastercardAmount = $response->getAmount();
                if ($visamastercardAmount!=$orderAmount && false)
                {
                    // TODO: create more stuff to handle this.
                    $order->addStatusToHistory(
                        $order->getStatus(),//continue setting current order status
                        Mage::helper('visamastercard')->__('Order total amount does not match visamastercard gross total amount')
                    );
                    $order->save();
                }
                else
                {
                    ...
                    if (!$order->canInvoice()) {
                       ...
                       $order->setState(
                           ...
                           Mage::helper('visamastercard')->__('Invoice #%s created', $invoice->getIncrementId()),
                           $notified = true
                       );
                    }
                    $order->save();
    ...
    public function getReturnUrl() {
        return Mage::getUrl('visamastercard/visamastercard/return');
    }
    
    public function getReportUrl() {
        return Mage::getUrl('visamastercard/visamastercard/report');
    }
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $stateObject->setStatus('pending_visamastercard');
