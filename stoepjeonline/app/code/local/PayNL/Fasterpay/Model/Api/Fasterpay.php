...
class PayNL_Fasterpay_Model_Api_Fasterpay extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 100;   // 1.00 GBP
    const     MAX_TRANS_AMOUNT = 20000; // 200.00 GBP
    const     PAY_PAYMENT_PROFILE_ID = 610; // 610 = fasterpay
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/fasterpay_fasterpay/'.$key);
    ...
    public function createPayment ($amount, $return_url, $report_url, $emailAddress, $orderData)
    {
        ...
        if (  !$this->setAmount($amount))
        {
            ...
            echo $amount, $return_url, $report_url;
            return false;
        }
	...
	$arguments['consumerIp'] = $this->consumer_ip;
        ...
        $arguments['currencyId'] = 1; // Default input are euro's which will be recalculated to GBP later on
