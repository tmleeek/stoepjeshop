<?php
/**
 * Pay.nl Poverboeking checkout model
 *
 * @category    PayNL
 ...
 */
class PayNL_Poverboeking_Model_Poverboeking extends Mage_Payment_Model_Method_Abstract
{
    const PAYMENT_TYPE_SALE = 'SALE';
    protected $_code  = 'poverboeking_poverboeking';
    protected $_formBlockType = 'poverboeking/poverboeking_form';
    protected $_infoBlockType = 'poverboeking/poverboeking_info';
    ...
    {
          return Mage::getUrl('poverboeking/poverboeking/redirect', array('_secure' => true));
    }

    /**
     * Get Pay.nl poverboeking api Model
     *
     * @return PayNL_Poverboeking_Model_Api_Poverboeking
     */
    public function getApi()
    {
        return Mage::getSingleton('poverboeking/api_poverboeking');
    }

    /**
     * Make sure only the correct currency's are allowed for poverboeking...
     * That being EUR only.
     *
     * @return bool
     */
    public function validate()
    {
        ...
        if (!in_array($currency_code,$this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('poverboeking')->__('Selected currency code ('.$currency_code.') is not compatible with Overboeking'));
        }
        return $this;
    }
    ...
    public function checkPayment($transactionId, $action="")
    {
        ...
        $order->loadByAttribute('poverboeking_transaction_id', $transactionId);
            ...
            else
            {
                ...
                if (in_array($action,$arrActionDel))
                {
                    ...
                    $order->addStatusToHistory(
                                    ...
                                    Mage::helper('poverboeking')->__('Refund', true),
                                    false);
                    ...
                    $mail->setSubject(Mage::helper('poverboeking')->__('Order #%s refunded', $order->getRealOrderId()));
                    $mail->setFrom($fromaddr,$fromname);

                    $body = Mage::helper('poverboeking')->__("The order: #%s has been refunded.\nPay.nl information:\nOrder code: %s\nNo further notifications for this will be available.",$order->getRealOrderId(),$transactionId);
                    $mail->setBodyText($body);
                    $mail->send();
                    return true;
                }
                $order->setPoverboekingPaidStatus(1);
                $orderAmount = $order->getBaseGrandTotal()*100;
                $poverboekingAmount = $response->getAmount();
                if ($poverboekingAmount!=$orderAmount && false)
                {
                    // TODO: create more stuff to handle this.
                    $order->addStatusToHistory(
                        $order->getStatus(),//continue setting current order status
                        Mage::helper('poverboeking')->__('Order total amount does not match overboeking gross total amount')
                    );
                    $order->save();
                }
                else
                {
                    ...
                    if (!$order->canInvoice()) {
                       ...
                       $order->setState(
                           ...
                           Mage::helper('poverboeking')->__('Invoice #%s created', $invoice->getIncrementId()),
                           $notified = true
                       );
                    }
                    $order->save();
		...
		if ($response->getFinal())
		{
                ...
                $order->setPoverboekingPaidStatus(0);
                $order->save();
            }
        }
    }
    ...
    public function getReturnUrl() {
        return Mage::getUrl('poverboeking/poverboeking/return');
    }
    
    public function getReportUrl() {
        return Mage::getUrl('poverboeking/poverboeking/report');
    }
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $stateObject->setStatus('pending_poverboeking');
