...
class PayNL_Poverboeking_Model_Api_Poverboeking extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 136; // 136 = poverboeking
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/poverboeking_poverboeking/'.$key);
	...
	$arguments['consumerIp'] = $this->consumer_ip;
        ...
        $arguments['consumerEmail'] = $emailAddress;
