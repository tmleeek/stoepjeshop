...
class PayNL_Pincasso_Model_Api_Pincasso extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 137; // 137 = pincasso
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/pincasso_pincasso/'.$key);
