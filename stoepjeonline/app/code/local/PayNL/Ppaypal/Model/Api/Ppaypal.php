...
class PayNL_Ppaypal_Model_Api_Ppaypal extends Varien_Object
{
    const     MIN_TRANS_AMOUNT = 10;   // 0.10 euro
    const     MAX_TRANS_AMOUNT = 1000000; // 10000.00 euro
    const     PAY_PAYMENT_PROFILE_ID = 138; // 138 = ppaypal
    ...
    {
        if (!$this->hasData($key)) {
             $value = Mage::getStoreConfig('payment/ppaypal_ppaypal/'.$key);
