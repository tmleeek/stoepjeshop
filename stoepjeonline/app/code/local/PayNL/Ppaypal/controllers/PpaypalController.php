...
class PayNL_Ppaypal_PpaypalController extends Mage_Core_Controller_Front_Action
{
    ...
    public function getPpaypal()
    {
        return Mage::getSingleton('ppaypal/ppaypal');
    }
    /*
    ...
    public function redirectAction()
    {
        ...
        if($order->getId())
        {
            if(strlen($order->getPpaypalTransactionId()) == 0)
            {
              ...
              if ($response)
              {
                  $session->setPpaypalPpaypalQuoteId($session->getQuoteId());
                  $order->setPpaypalTransactionId($response->getTransactionId());
                  $order->setPpaypalPaidStatus(0);
                  $order->save();

                  $this->getResponse()->setBody(
                      $this->getLayout()->createBlock('ppaypal/ppaypal_redirect')
	...
	$orderApi = Mage::getSingleton('ppaypal/api_ppaypal')->checkPayment($transactionId);
        $order = Mage::getModel('sales/order');
        ...
        if ($orderApi->getPaidStatus() != true)
        {
            ...
            $session->setQuoteId($session->getPpaypalPpaypalQuoteId(true));
	...
	$transaction_id = $this->getRequest()->get('order_id');
        ...
        $this->getPpaypal()->checkPayment($transaction_id,$action);
	echo "true|done";
    }
