...
class MageWorx_Adminhtml_Block_Multifees_Fee_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_updateButton('save', 'label', Mage::helper('multifees')->__('Save Fee'));
        $this->_updateButton('delete', 'label', Mage::helper('multifees')->__('Delete Fee'));
    ...
    public function getHeaderText()
    {
        if (Mage::registry('multifees_data') && Mage::registry('multifees_data')->getId()) {
            return Mage::helper('multifees')->__("Edit Fee '%s'", $this->htmlEscape(Mage::getSingleton('multifees/language_fee')->load(Mage::registry('multifees_data')->getId(), 'mfl_fee_id')->getTitle()));
        } else {
            return Mage::helper('multifees')->__('Add Fee');
        }
    }
}