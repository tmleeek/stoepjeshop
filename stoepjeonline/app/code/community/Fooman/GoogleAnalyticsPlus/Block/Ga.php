...
';
	...
	{
		$html = parent::getQuoteOrdersHtml();
		if ($accountId2) {
			$html .= "\n".str_replace('_gaq.push(["_', '_gaq.push(["t2._', $html);
		}
		if($html){
		//set customer variable for the current session c=1
		$html .= "_gaq.push(['_setCustomVar', 5, 'c', '1', 2]);";
		//set repeat customer variable onwards for this visitor rc=1
		$html .= "_gaq.push(['_setCustomVar', 5, 'rc', '1', 1]);";
		}
		return $html;
	}
	...
	{
		$html = parent::_getOrdersTrackingCode();
		if ($accountId2) {
			$html .= "\n".str_replace('_gaq.push(["_', '_gaq.push(["t2._', $html);
		}
		//set customer variable for the current session c=1
