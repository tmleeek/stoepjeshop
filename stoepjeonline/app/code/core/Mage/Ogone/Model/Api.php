 ...
 */
        $secretCode = $this->getConfig()->getShaOutCode();
        $secretSetFields = array();
	foreach ($formFields as $key => $val) {
		if ($this->_translate($val))
			$secretSetFields[strtoupper($key)] = strtoupper($key) . '=' . $this->_translate($val) . $secretCode;
	}
	ksort($secretSetFields);
//$formFields['_______']  = str_replace($secretCode, '', implode('___', $secretSetFields));
        $formFields['SHASign']  = Mage::helper('ogone')->shaCrypt(implode('', $secretSetFields));
/* BUGFIX END */
        return $formFields;
    }
